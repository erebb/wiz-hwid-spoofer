...
# ── macOS / Wine: devre dışı bırakılan loop'lar ──────────────────────────────
# Discord RPC, ban kontrolü, drop logging ve anti-afk hareketi macOS Wine'da ya
# çalışmaz ya da gereksiz CPU/ısı yükü getirir. Aşağıdaki loop'lar erken döner.
rpc_status = False
drop_status = False
anti_afk_status = False
...
async def main():
		...
		async def async_in_combat(client: Client):
			...
			while True:
				...
				if not freecam_status:
					...
					# macOS: savaş bitişini say → 3 savaşta bir iksir (potion_usage_loop içinde)
					if was_in_combat and not client.in_combat:
						client.mac_battle_count = getattr(client, 'mac_battle_count', 0) + 1
						logger.debug(f'{client.title} - Savaş bitti, iksir sayacı: {client.mac_battle_count}/3')
	...
	async def anti_afk_loop():
		...
		# macOS: anti-afk hareketi devre dışı (ısı/performans).
		return
	...
	async def _init_client_attrs(client):
		...
		client.mac_battle_count = 0  # macOS: 3 savaşta bir iksir sayacı
	...
	async def potion_usage_loop():
		...
		async def async_potion(client: Client):
			while True:
				...
				if freecam_status:
					continue
				# macOS: 3 savaş sonrası otomatik iksir (questing/sigil dahil), satın alma yok
				if getattr(client, 'mac_battle_count', 0) >= 3 and await is_free(client):
					logger.info(f'{client.title} - 3 savaş tamamlandı, iksir kullanılıyor (satın alma yok).')
					try:
						await auto_potions(client, buy=False)
					except Exception as e:
						logger.debug(f'{client.title} - iksir hatası atlandı: {e}')
					client.mac_battle_count = 0
				# GUI auto potion toggle (savaş dışında manuel)
				elif use_potions and auto_potion_status and await is_free(client) and not any([client.sigil_status, client.questing_status]):
					await auto_potions(client, buy=False)
	...
	async def rpc_loop():
		# macOS: Discord Rich Presence devre dışı (Discord IPC Wine'da yok).
		return
		if not rpc_status:
			return
	...
	async def drop_logging_loop():
		# macOS: drop logging devre dışı (performans).
		return
		# Auto potion usage on a per client basis.
	...
	async def ban_watcher():
		# macOS: ban kontrolü devre dışı (Discord IPC Wine'da yok, hatalı fatal popup riski).
		return
		known_ban = False
		try:
