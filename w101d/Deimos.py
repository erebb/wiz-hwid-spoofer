...
# 2. Traversal Data Hard-Patch
current_dir = os.path.dirname(os.path.abspath(__file__))
traversal_folder = os.path.join(current_dir, "traversalData")
if not os.path.exists(traversal_folder):
...
if os.path.exists(mac_silicon_path):
    ...
    logger.debug(f"Mac Tesseract (Silicon) Yolu Ayarlandı: {mac_silicon_path}")
elif os.path.exists(mac_intel_path):
    pytesseract.pytesseract.tesseract_cmd = mac_intel_path
    logger.debug(f"Mac Tesseract (Intel) Yolu Ayarlandı: {mac_intel_path}")
else:
...
# 6. TELEPORT TIMEOUT BYPASS — sadece should_update beklemesini atla
import wizwalker.utils as _ww_utils
from wizwalker.client import Client as _WClient

# 6a. maybe_wait_for_value_with_timeout → sadece should_update için anında return
# Diğer wait'lere (zone load, in_battle, vs.) dokunma — loading ekranı sıkışır
_orig_maybe_wait = _ww_utils.maybe_wait_for_value_with_timeout
async def _selective_wait(coro_func, *args, **kwargs):
    name = getattr(coro_func, '__name__', '') or ''
    if 'should_update' in name:
        logger.debug("[macOS] should_update atlandı (selective bypass)")
        return True
    return await _orig_maybe_wait(coro_func, *args, **kwargs)
_ww_utils.maybe_wait_for_value_with_timeout = _selective_wait
logger.debug("[macOS] should_update bypass aktif (diğer wait'ler korunuyor)")

# 6b. Client.teleport → wait_on_inuse=False (nav mesh hesabına dokunma)
# body.write_position kullanmıyoruz — navmap_tp nav mesh pozisyonunu hesaplar,
# teleport sadece should_update beklemesini atlar
...
async def _teleport_nowait(self, xyz, yaw=None, wait_on_inuse=True, **kwargs):
    try:
        ...
        logger.warning(f"[macOS] teleport HATA ({_te})")
...
def read_config(config_name : str):
	...
	global gui_on_top, gui_theme, gui_text_color, gui_button_color, gui_langcode, gui_scale, gui_font, gui_font_size
	...
	global use_team_up, buy_potions, client_to_follow
	...
	global client_to_boost, questing_friend_tp, gear_switching_in_solo_zones, hitter_client
...
files_to_download = [
	"displayZones.txt", "gates_list.txt", "interactiveTeleporters.txt", 
	"objectLocations.txt", "uniqueObjectLocations.txt", "zoneMap.txt"
]
base_url = "https://raw.githubusercontent.com/notfaj/wizsprinter/main/wizwalker/extensions/wizsprinter/traversalData/"

for file_name in files_to_download:
	local_path = os.path.join(traversal_folder, file_name)
	if not os.path.exists(local_path):
		try:
			logger.debug(f"İndiriliyor: {file_name}...")
			download_file(base_url + file_name, local_path)
		except Exception as e:
			pass
	...
	async def hooking_logic(default_logic : bool = False):
		...
		if not default_logic:
			...
			for i, p in enumerate(walker.clients, 1):
				...
				if not hasattr(p, "questing_status"):
